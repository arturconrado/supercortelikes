...
export function Card({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) { return <div className={cn('min-w-0 rounded-2xl border border-white/[.08] bg-panel/80 shadow-[0_20px_50px_rgba(0,0,0,.14)]', className)} {...props} />; }
...
export function PageHeader({ eyebrow, title, description, action }: { eyebrow?: string; title: string; description?: string; action?: React.ReactNode }) { return <div className="mb-7 flex min-w-0 flex-col justify-between gap-4 sm:flex-row sm:items-end"><div className="min-w-0">{eyebrow && <p className="mb-2 break-words text-xs font-bold uppercase tracking-[.18em] text-lime">{eyebrow}</p>}<h1 className="break-words text-2xl font-bold tracking-tight text-white sm:text-3xl">{title}</h1>{description && <p className="mt-2 max-w-2xl break-words text-sm leading-6 text-zinc-500">{description}</p>}</div>{action && <div className="min-w-0 max-w-full [&>*]:flex-wrap">{action}</div>}</div>; }
export function MetricCard({ label, value, detail, icon: Icon }: { label: string; value: string | number; detail?: string; icon: React.ComponentType<{ className?: string }> }) { return <Card className="p-5"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><p className="break-words text-xs font-medium uppercase tracking-wider text-zinc-500">{label}</p><p className="mt-3 break-words text-2xl font-bold tracking-tight text-white">{value}</p>{detail && <p className="mt-1 break-words text-xs text-zinc-500">{detail}</p>}</div><div className="shrink-0 rounded-xl bg-lime/[.08] p-2.5"><Icon className="size-5 text-lime" /></div></div></Card>; }
export function Modal({ open, onClose, title, description, children }: { open: boolean; onClose: () => void; title: string; description?: string; children: React.ReactNode }) {
  ...
  if (!open) return null;
  return <div className="fixed inset-0 z-[70] grid place-items-center p-4"><button tabIndex={-1} className="absolute inset-0 bg-black/75 backdrop-blur-sm" onClick={onClose} aria-label="Fechar janela"/><div ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="modal-title" aria-describedby={description ? 'modal-description' : undefined} className="relative max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto animate-fade-in rounded-2xl border border-white/10 bg-[#111319] p-6 shadow-2xl"><button onClick={onClose} className="absolute right-4 top-4 rounded-lg p-1.5 text-zinc-500 hover:bg-white/[.06] hover:text-white" aria-label="Fechar"><X className="size-4"/></button><h2 id="modal-title" className="pr-8 text-xl font-bold text-white">{title}</h2>{description && <p id="modal-description" className="mt-2 text-sm leading-6 text-zinc-500">{description}</p>}<div className="mt-6">{children}</div></div></div>;
}
